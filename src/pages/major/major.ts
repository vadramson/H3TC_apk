...
export class MajorPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public alertCtrl: AlertController, public h3tcApiProvider: H3tcApiProvider, public events: Events, public app: App,
   ...
   dropMajor(ordIDNum){
    if (ordIDNum){
      ...
      this.h3tcApiProvider.postData(this.majDel, "drop/major/courses/").then((result) => {
        this.responseData = result;
        if (this.responseData.major) {
          this.my_majorCourses=this.responseData.major;
  ...
  addMajor() {
    if(this.majorCourseData.user && this.majorCourseData.id){
      this.h3tcApiProvider.postData(this.majorCourseData, "add/major/courses/").then((result) => {
        this.responseData = result;
  ...
  loadMajorCourses() {
    ...
    this.h3tcApiProvider.postData(this.courserData, "get/all/major/courses/").then((result) => {
      this.responseData = result;
      if (this.responseData.major) {
        this.my_majorCourses=this.responseData.major;
        console.log("this.my_majorCourses");
