...
/**
 * Generated class for the ElectivePage page.
 *
...
export class ElectivePage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public alertCtrl: AlertController, public h3tcApiProvider: H3tcApiProvider, public events: Events, public app: App,
   ...
   dropElective(ordIDNum){
    if (ordIDNum){
      this.majDel.id = ordIDNum;
      console.log(this.majDel);
      this.h3tcApiProvider.postData(this.majDel, "drop/elective/courses/").then((result) => {
        this.responseData = result;
        if (this.responseData.elective) {
          this.loadElectiveCourses();
          this.my_majorCourses=this.responseData.elective;
  ...
  addElective() {
    if(this.majorCourseData.user && this.majorCourseData.id){
      this.h3tcApiProvider.postData(this.majorCourseData, "add/elective/courses/").then((result) => {
        this.responseData = result;
  ...
  loadElectiveCourses() {
    ...
    this.h3tcApiProvider.postData(this.courserData, "get/all/elective/courses/").then((result) => {
      this.responseData = result;
      if (this.responseData.elective) {
        this.my_majorCourses=this.responseData.elective;
        console.log("this.my_majorCourses");
